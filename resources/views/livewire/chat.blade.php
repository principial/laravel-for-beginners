<div x-data="{isOpen: false}">
    <span x-on:click="isOpen = true; document.querySelector('.chat-field').focus()"
          class="text-white mr-2 header-chat-icon" title="Chat" data-toggle="tooltip"
    ...
    <div data-username="{{auth()->user()->username}}" data-avatar="{{auth()->user()->avatar}}" id="chat-wrapper"
        ...
        <div x-on:click="isOpen = false" class="chat-title-bar">Chat <span class="chat-title-bar-close"><i
                    ...
                    @else
                        <div class="chat-other">
                            ...
                            <div class="chat-message"><div class="chat-message-inner">
                                    <a wire:navigate href="/profile/{{$chat['username']}}"><strong>{{$chat['username']}}:</strong></a>
                                    {{$chat['textvalue']}}
                                </div></div>
                        </div>
                    @endif
